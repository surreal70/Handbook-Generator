...
class QualityControlOrchestrator:
    ...
    reports. It also tracks metrics over time and provides trend analysis.
    """
    ...
    def analyze_trends(self, current_report: QualityReport) -> Dict[str, Dict[str, float]]:
        """
        Compare current metrics with previous run and identify trends.
        
        Compares current metrics with the previous run, identifies improvements
        and regressions, and calculates percentage changes.
        
        Args:
            current_report: Current QualityReport object
        ...
        if not self.metrics_history_file.exists():
            self.logger.info("No previous metrics found for trend analysis")
            return trends
        ...
        if not history.get("runs") or len(history["runs"]) < 1:
            self.logger.info("No previous runs found for trend analysis")
            return trends
        
        # Get the most recent previous run (last one in history before current)
        ...
        for metric_name in metrics_to_analyze:
            current_value = getattr(current_report.metrics, metric_name)
            previous_value = previous_run.get(metric_name, 0.0)
            
            change = current_value - previous_value
            
            # Determine trend
            if abs(change) < 0.1:  # Less than 0.1% change is considered stable
                trend = 'stable'
            elif change > 0:
                trend = 'improved'
            else:
    ...
    def generate_trend_report(self, trends: Dict[str, Dict[str, float]]) -> str:
        """
        Generate a human-readable trend analysis report.
        
        Args:
            trends: Dictionary with trend analysis from analyze_trends()
            
        Returns:
            Formatted trend report as a string
        """
        if not trends:
            return "No previous metrics available for trend analysis."
        
        lines = []
        lines.append("=" * 80)
        lines.append("QUALITY METRICS TREND ANALYSIS")
        lines.append("=" * 80)
        lines.append("")
        ...
        if improvements:
            ...
            for metric_name, data in improvements:
                display_name = metric_name.replace('_', ' ').title()
                lines.append(
                    f"  • {display_name}: {data['previous']:.1f}% → {data['current']:.1f}% "
                    f"(+{data['change']:.1f}%)"
                )
            lines.append("")
        ...
        regressions = [
            (name, data) for name, data in trends.items()
            if data['trend'] == 'declined'
        ]
        
        if regressions:
            lines.append("⚠ REGRESSIONS:")
            for metric_name, data in regressions:
                display_name = metric_name.replace('_', ' ').title()
                lines.append(
                    f"  • {display_name}: {data['previous']:.1f}% → {data['current']:.1f}% "
                    f"({data['change']:.1f}%)"
        ...
        if stable:
            lines.append("→ STABLE:")
            for metric_name, data in stable:
                display_name = metric_name.replace('_', ' ').title()
                lines.append(f"  • {display_name}: {data['current']:.1f}%")
            lines.append("")
        
        # Summary
        if improvements and not regressions:
            lines.append("Overall: Quality metrics have improved! 🎉")
        elif regressions and not improvements:
            lines.append("Overall: Quality metrics have declined. Investigation recommended.")
        elif improvements and regressions:
            lines.append("Overall: Mixed results. Some metrics improved, others declined.")
        else:
            lines.append("Overall: Quality metrics remain stable.")
