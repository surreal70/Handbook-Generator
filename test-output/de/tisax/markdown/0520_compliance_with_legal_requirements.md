
Document-ID: tisax-0520

Status: Draft
Classification: Internal

# Einhaltung gesetzlicher Anforderungen

**Dokument-ID:** [FRAMEWORK]-0520
**Organisation:** AdminSend GmbH
**Owner:** [TODO]
**Genehmigt durch:** [TODO]
**Revision:** [TODO]
**Author:** Handbook-Generator
**Status:** Draft
**Klassifizierung:** Internal
**Letzte Aktualisierung:** [TODO]
**Template Version:** [TODO]

---

---

## Zweck

Dieses Dokument beschreibt die Maßnahmen zur Einhaltung gesetzlicher Anforderungen gemäß TISAX-Anforderungen.

## Geltungsbereich

Dieses Dokument gilt für alle Aktivitäten von [TODO].

## Relevante Gesetze und Vorschriften

### Datenschutz

**DSGVO (Datenschutz-Grundverordnung):**
- Rechtmäßigkeit der Verarbeitung
- Betroffenenrechte
- Meldepflichten
- Technische und organisatorische Maßnahmen

**BDSG (Bundesdatenschutzgesetz):**
- Nationale Ergänzungen zur DSGVO
- Besondere Verarbeitungssituationen

### Informationssicherheit

**IT-Sicherheitsgesetz:**
- Kritische Infrastrukturen
- Meldepflichten
- Sicherheitsstandards

**BSI-Grundschutz:**
- Empfohlene Sicherheitsmaßnahmen
- Best Practices

### Branchenspezifisch

**Automotive:**
- TISAX-Anforderungen
- VDA ISA Katalog
- Kundenspezifische Anforderungen

## Compliance-Management

### Compliance-Organisation

**Verantwortlichkeiten:**
- Compliance Officer: [TODO]
- Datenschutzbeauftragter: [TODO]
- Informationssicherheitsbeauftragter: [TODO]

**Aufgaben:**
- Überwachung der Compliance
- Beratung
- Schulung
- Reporting

### Compliance-Prozesse

**Identifikation:**
- Relevante Gesetze und Vorschriften
- Regulatorische Änderungen
- Kundenanforderungen
- Branchenstandards

**Bewertung:**
- Gap-Analyse
- Risikobewertung
- Priorisierung
- Maßnahmenplanung

**Umsetzung:**
- Implementierung von Maßnahmen
- Dokumentation
- Schulung
- Überwachung

**Überprüfung:**
- Regelmäßige Audits
- Compliance-Checks
- Reporting
- Verbesserungsmaßnahmen

## Meldepflichten

### Datenschutzverletzungen

**DSGVO-Meldepflicht:**
- An Aufsichtsbehörde: Innerhalb 72 Stunden
- An Betroffene: Ohne unangemessene Verzögerung
- Dokumentation aller Vorfälle

### Sicherheitsvorfälle

**IT-Sicherheitsgesetz:**
- Meldung an BSI bei kritischen Infrastrukturen
- Zeitnahe Meldung
- Dokumentation

### Vertragliche Meldepflichten

**TISAX:**
- Meldung an Kunden gemäß Vertrag
- Zeitnahe Information
- Umfassende Dokumentation

## Dokumentation

### Verzeichnis von Verarbeitungstätigkeiten

**DSGVO-Anforderung:**
- Alle Verarbeitungstätigkeiten dokumentieren
- Regelmäßige Aktualisierung
- Bereitstellung für Aufsichtsbehörde

### Compliance-Nachweise

**Dokumentation:**
- Richtlinien und Verfahren
- Schulungsnachweise
- Audit-Berichte
- Zertifikate
- Verträge

## Schulung und Awareness

### Schulungsprogramm

**Zielgruppen:**
- Alle Mitarbeiter: Basis-Schulung
- Führungskräfte: Erweiterte Schulung
- Spezialisten: Fachspezifische Schulung

**Inhalte:**
- Datenschutz
- Informationssicherheit
- Compliance-Anforderungen
- Meldepflichten

**Frequenz:**
- Jährlich für alle Mitarbeiter
- Bei Einstellung
- Bei Änderungen

## Audits und Überprüfungen

### Interne Audits

**Regelmäßig:**
- Jährliche Compliance-Audits
- Quartalsweise Stichproben
- Ad-hoc bei Bedarf

**Prozess:**
1. Planung
2. Durchführung
3. Berichterstattung
4. Maßnahmenplanung
5. Follow-up

### Externe Audits

**Durch:**
- Aufsichtsbehörden
- Zertifizierungsstellen
- Kunden
- Externe Auditoren

**Vorbereitung:**
- Dokumentation bereitstellen
- Ansprechpartner benennen
- Räumlichkeiten vorbereiten

## TISAX-spezifische Anforderungen

### VDA ISA Kontrollen

Dieses Dokument adressiert:
- **11.1**: Einhaltung gesetzlicher Anforderungen

### Assessment-Nachweise

- Compliance-Dokumentation
- Schulungsnachweise
- Audit-Berichte
- Zertifikate

## Kennzahlen

[TODO] misst:
- Anzahl Compliance-Verstöße
- Schulungsteilnahme
- Anzahl durchgeführter Audits
- Anzahl offener Findings

