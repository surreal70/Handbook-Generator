...
This template documents the security objectives for the TOE and its environment according to ISO/IEC 15408 (Common Criteria).

Security objectives describe the intended security properties of the TOE and its operational environment.
They are derived directly from the security problem definition (threats, OSPs, assumptions).

Structure:
- Security objectives for the TOE (O.xxx): What the TOE itself must achieve
- Security objectives for the environment (OE.xxx): What the operational environment must provide

Each security objective should:
- Be uniquely identifiable (O.ID or OE.ID)
- Be clearly and precisely formulated
- Respond to specific threats, OSPs, or assumptions
- Be measurable or verifiable

Best Practices:
- Use consistent naming conventions (O.ACCESS_CONTROL, O.AUDIT, etc.)
- Avoid technical implementation details (these belong in Security Requirements)
- Ensure all threats are addressed by objectives
- Document the rationale (justification) for each objective
-->
...
<!-- 
GUIDANCE: List all security objectives that the TOE itself must fulfill.
Each objective should have a unique ID (O.xxx) and clearly describe what should be achieved.
-->
...
<!-- 
GUIDANCE: List all security objectives that must be fulfilled by the operational environment.
These objectives typically address assumptions about the environment and may include physical security,
personnel, external systems, or organizational measures.
-->
